...
- **aurum2 CUDA build tree:** `/home/yesylevskyy/install/gromacs-2026-cph-gpu` (`build-cuda`) — an rsync of *this* tree used to build/test CUDA (there is no local GPU). See [[reference_aurum2_cuda_build]] and "Building & running on aurum2 (GPU)" below. Re-sync after local edits; **exclude `build-*` not `build*`** (the latter drops `src/buildinfo.h.cmakein`). Not a git repo — sync files, and note it still carries the dropped L2.2 device-reduce WIP.
- **aurum2 CPU build tree:** `/home/yesylevskyy/install/gromacs-2026-cph` (`build-sys`, plus `build_sys.sh`/`build_cuda.sh` wrappers that `spack load` and touch `BUILD_DONE`/`BUILD_FAIL`). This one **is** a git clone of `origin`, so update it with a **git bundle** rather than an rsync: `git bundle create x.bundle <theirHEAD>..master` locally, scp it, then `git fetch /path/x.bundle master && git merge --ff-only FETCH_HEAD`. That keeps history and avoids pushing to `origin` unasked. ‼️ It had silently drifted **29 commits behind** (pre-L0.1, i.e. no CPU PME reciprocal potential) — a bug report produced with its binary cost an extra debugging round. **Check `git log -1` there before trusting any result from it.**
...
- Tree: `/home/yesylevskyy/install/gromacs-2026-cph-gpu` (rsync of this tree; `--exclude 'build-*' --exclude '.git'`).
- Toolchain: `spack load cmake@3.31.11 fftw@3.3.10 cuda@13.1.1` (host gcc = system 11.5).
- Configure: `cmake -S . -B build-cuda -DGMX_GPU=CUDA -DGMX_MPI=OFF -DGMX_DOUBLE=OFF -DGMX_FFT_LIBRARY=fftw3 -DBUILD_TESTING=OFF -DGMX_CUDA_TARGET_SM=86 -DCMAKE_BUILD_TYPE=Release -DGMX_SIMD=AVX2_256`.
...
- **‼️ CPU-PME on GPU nodes needs `GMX_FFT_LIBRARY=fftpack`** (the spack fftw is AVX-512 → SIGILLs on Zen3).
