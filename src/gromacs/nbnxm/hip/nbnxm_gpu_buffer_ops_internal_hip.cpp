 ...
 */
static __global__ void nbnxmKernelTransformXToXq(int numColumns,
    ...
    if (cxy < numColumns)
    {
        ...
        if (threadIndex < numAtoms)
        {
            ...
            *gm_xqDest          = gm_x[atomIndex];
            /* Constant-pH (GPU-resident path): the X buffer-op is the only per-step writer of the
             * nbnxm xq buffer (no full x+q H2D), so it must also refresh the charge in the .w
             * component from the current lambda-interpolated charges. gm_charge is nullptr for
             * non-constant-pH runs, leaving .w untouched (bit-identical to before). Mirrors
                                     ...
                                     const int            gridId)
{
    ...
    auto       kernelFn      = nbnxmKernelTransformXToXq;
